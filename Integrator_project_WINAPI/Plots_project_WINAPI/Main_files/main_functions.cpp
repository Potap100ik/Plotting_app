...
void MousePos(BOOL fl, int x, int y, MouseMove& MyMouse)
{
...
{
	Wnd_Plot->setka.Left_Wide_Vec_UNIT = Wnd_Plot->setka.u_x_Left - SYSTEMMETRICS * Wnd_Plot->setka.unit_to_pixel;
	Wnd_Plot->setka.Right_Wide_Vec_UNIT = Wnd_Plot->setka.u_x_Left + SYSTEMMETRICS * Wnd_Plot->setka.unit_to_pixel;
...
{//ПЕРВОЕ ЗАПОЛНЕНИЕ ДАННЫХ О ФУНКЦИИ
	...
	for (double x = Wnd_Plot->setka.Left_Wide_Vec_UNIT;
		...
		catch (char char_err)
		{
			...
			swprintf(str, L"Найден запрещенный символ: %s", char_err);
...
double FillIntegralVector(Integral_struct& Myintegr, MyWnd_Plot* Wnd_Plot, double & correct_x , double& correct_y)
{
	...
	double x = Myintegr.vec.myvec_xy[0].x - Wnd_Plot->setka.unit_to_pixel;
	double y = 0;
...
void StructPlotInit(MyWnd_Plot* Wnd_Plot, Ploting_struct& Myplot)
{
	...
	Wnd_Plot->setka.Left_Wide_Vec_UNIT = -SYSTEMMETRICS * Wnd_Plot->setka.unit_to_pixel;
	Wnd_Plot->setka.Right_Wide_Vec_UNIT = Wnd_Plot->setka.Left_Wide_Vec_UNIT +
		Wnd_Plot->setka.Wide_for_Vec_capasity * Wnd_Plot->setka.unit_to_pixel;
...
void StructIntegralInit(MyWnd_Plot* Wnd_Integral, Integral_struct& Myintegr)
{
	...
	Wnd_Integral->setka.Wide_for_Vec_capasity = (Myintegr.B - Myintegr.A)/Wnd_Integral->setka.unit_to_pixel + 1;

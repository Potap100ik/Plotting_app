...
char* StrCut(char* str, double* x, int y, int const index, FUNC en) {//манипуляции с выражением в скобках - режь строку, вычисляй скобки, склей строку, верни строку на место
	...
	bool fl;
	fl = (strncmp(str1, "+nan(ind)", 4) == 0 || strncmp(str1, "-nan(ind)", 4) == 0 || strncmp(str1, "nan", 3) == 0 || strcmp(str1, "nan)") == 0 || strncmp(str1, "-inf", 3) == 0 || strncmp(str1, "+inf", 3) == 0 || strncmp(str1, "inf", 3) == 0);
	if (fl)
	{
		throw(str1);
	}
